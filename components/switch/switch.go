...
func New(props Props) g.Node {
	...
	// Thumb classes - we'll use CSS to handle the translation based on checked state
	thumbClasses := lib.CN(
	...
	// Create style element for dynamic checked state
	// This is a workaround since we can't use Radix UI's state management
	switchStyles := html.Style(`
			.switch-track:has(input:checked) {
				background-color: hsl(var(--primary));
			}
			.switch-track:has(input:not(:checked)) {
				background-color: hsl(var(--input));
			}
			.switch-track:has(input:checked) .switch-thumb {
				transform: ` + thumbTranslate + `;
			}
			.switch-track:has(input:not(:checked)) .switch-thumb {
	...
	return g.Group{
		...
		html.Label(
			...
			html.Span(
				html.Class(lib.CN(trackClasses, "switch-track")),
				html.Role("switch"),
				...
				html.Span(html.Class(lib.CN(thumbClasses, "switch-thumb"))),
			),
		),
	}
}
