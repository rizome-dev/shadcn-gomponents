...
func TestNew(t *testing.T) {
	t.Run("creates basic chart with default options", func(t *testing.T) {
		...
		if !strings.Contains(html, `data-chart="true"`) {
			t.Error("expected chart data attribute")
		}
		...
		if !strings.Contains(html, "Loading chart...") {
	...
	t.Run("creates chart with title and subtitle", func(t *testing.T) {
		...
		if !strings.Contains(html, `data-title="Chart Title"`) {
			t.Error("expected title attribute")
		}
		if !strings.Contains(html, `data-subtitle="Chart Subtitle"`) {
			t.Error("expected subtitle attribute")
		}
	...
	t.Run("hides legend when requested", func(t *testing.T) {
		...
		if !strings.Contains(html, `data-hide-legend="true"`) {
			t.Error("expected hide-legend attribute")
		}
	})
	...
	t.Run("hides grid when requested", func(t *testing.T) {
		...
		if !strings.Contains(html, `data-hide-grid="true"`) {
			t.Error("expected hide-grid attribute")
		}
	})
	...
	t.Run("disables animations when requested", func(t *testing.T) {
		...
		if !strings.Contains(html, `data-no-animations="true"`) {
			t.Error("expected no-animations attribute")
		}
	})
	...
	t.Run("embeds chart data as JSON", func(t *testing.T) {
		...
		if !strings.Contains(html, `data-chart-data=`) {
			t.Error("expected chart data attribute")
		}
		if !strings.Contains(html, "Revenue") {
			t.Error("expected series name in data")
		}
	})
}
...
func TestCanvas(t *testing.T) {
	t.Run("creates canvas element in chart", func(t *testing.T) {
		...
		if !strings.Contains(html, `id="canvas-test-canvas"`) {
			t.Error("expected canvas ID")
		}
...
func TestLegend(t *testing.T) {
	t.Run("creates legend when present in chart", func(t *testing.T) {
		...
		// Since the standard chart uses canvas/JS, we can't check rendered legend
		// The legend will be rendered by the JavaScript library
		if !strings.Contains(html, `data-chart-data`) {
			t.Error("expected chart data for legend rendering")
		}
	})
}
