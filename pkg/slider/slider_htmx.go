	...
	for i, value := range props.Value {
		...
		thumb := html.Div(
			...
			html.Class(lib.CN(
				...
				"block h-4 w-4 shrink-0 rounded-full border-2 shadow",
				"transition-colors hover:cursor-grab active:cursor-grabbing",
				"focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
				"disabled:pointer-events-none disabled:opacity-50",
				"absolute z-10 cursor-grab active:cursor-grabbing",
			)),
			g.If(props.Orientation == "horizontal",
				g.Attr("style", fmt.Sprintf("left: calc(%.2f%% - 0.5rem); top: 50%%; transform: translateY(-50%%)", position)),
			),
			g.If(props.Orientation == "vertical",
				g.Attr("style", fmt.Sprintf("bottom: calc(%.2f%% - 0.5rem); left: 50%%; transform: translateX(-50%%)", position)),
			),
			g.If(props.Disabled,
